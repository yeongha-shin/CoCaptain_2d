...
def test_perceiver():
    ...
    # 한번의 학습에서 사용되는 데이터의 크기
    batch_size = 4
    # 모델 내부에서 사용되는 벡터의 차원
    model_dim = 128
    # 주변 정보가 사용될 때, 그 정보가 표현되는 차원, 보통 model_dim과 같은 차원으로 표현됨
    context_dim = 128
    # latent 변수의 개수, 학습 중에 숨겨진 트성 표현을 말함. 기억하거나 추론에 사용할 수 있는 정보다. 여기에 메모리로 사용된다고 생각해도 좋다
    num_latents = 16
    # 내부적으로 사용되는 transfomer 블록의 개수 ,
    num_blocks = 6
    # multi head attention에서 사용되는 head의 개수 , 여러개의 헤드를 사용하면 서로 다른 부분을 집중해서 사용할 수 있게 된다
    num_heads = 8
    # 입력데이터로부터 얻으려는 쿼리의 수, 관심을 두고 집중하려는 정보의 수 . 즉 이 개수만큼 중요한 정보를 알아내고 싶은 것이라고 한다
    num_queries = 4
    # 문맥의 길이. 이 개수의 토큰 만큼 정보를 추출하게 된다.
    context_len = 100  # number of tokens in context
    ...
    x = torch.randn(batch_size, num_latents, model_dim)  # Match num_latents with x_dim
    context = torch.randn(batch_size, context_len, context_dim)
    context_mask = torch.ones(batch_size, context_len).bool()

    # Forward pass
    y, attention_maps = perceiver(x, context, context_mask)

    # Test output shapes
    print("Output shape:", y.shape)  # should be (batch_size, num_queries, model_dim)
    print("Attention map shape:", attention_maps.shape)  # should be (batch_size, context_len)

    # Check if the output shape is as expected
    assert y.shape == (
    batch_size, num_queries, model_dim), f"Expected shape {(batch_size, num_queries, model_dim)}, but got {y.shape}"
    assert attention_maps.shape == (
    batch_size, context_len), f"Expected shape {(batch_size, context_len)}, but got {attention_maps.shape}"

    print("Perceiver test passed!")
