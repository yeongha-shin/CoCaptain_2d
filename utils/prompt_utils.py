...
# 사람이 이해할 수 있는 형태의 문장을 생성하는 함수다
# obs:이게 pkl데이터에 들어 있는 observation 데이터 이다. 또는
...
def load_pickle_data(filepath):
    """ Load pickle data from the provided file path """
    with open(filepath, 'rb') as file:
        data = pickle.load(file)
    return data

def run_test_with_pkl(filepath):
    data = load_pickle_data(filepath)

    for d in data:
        # print(d["observation"])
        input_prompt = make_observation_prompt(d["observation"])
        print(input_prompt)
        sys.exit()

if __name__ == "__main__":
    run_test_with_pkl("./data/vqa_test_1k.pkl")